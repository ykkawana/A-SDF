...
# max_poses_path = f'/home/mil/kawana/workspace/3detr/third_party/A-SDF/max_poses_{split}.pkl'
# with open(max_poses_path, 'rb') as f:
#     max_poses = pickle.load(f)
...
if not skip_intermediate:
    ...
    for pidx, path in tqdm(enumerate(paths)):
        ...
        # max_pose = max_poses[scene_id]
        # assert len(pkl['boxes']) == len(max_pose)
        for i, bbox in enumerate(pkl['boxes']):
            ...
            if bbox['type'] in [1, 0]:
                ...
                # print(bbox['type'], bbox['joint_info']['max'], max_pose[i])
            if bbox['type'] == 0:
    ...
    with open(f'../sapien_shapes_intermediate_{split}_fixed.pkl', 'wb') as f:
...
with open(f'/home/mil/kawana/workspace/3detr/third_party/A-SDF/sapien_shapes_{split}_fixed.pkl', 'wb') as f:
